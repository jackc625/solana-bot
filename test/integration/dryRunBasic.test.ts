...
process.env.PRIVATE_KEY =
  '5J7XKqzJkJJLnNcKtShDaVtSwqLLaUi3eRgHEJvBfpNP9uCNSTUn6KQVBqQU7U8CkMNWzTnbZqnuBvfAiYqJbSy';
...
jest.mock('../../src/utils/pumpTrade.js', () => ({
  sendPumpTrade: jest.fn().mockImplementation(async (params) => {
    console.log(
      `🔥 DRY RUN: PumpPortal ${params.action} - ${params.amount} SOL for ${params.mint}`,
    );
...
describe('Dry-Run Basic Validation', () => {
  ...
  describe('Transaction Prevention', () => {
    ...
    it('should prevent real Solana transactions', async () => {
      ...
      await expect(mockConnection.sendTransaction({})).rejects.toThrow(
        'DRY RUN: No real transactions',
      );
    });

    it('should log transaction attempts with details', () => {
      const tradeLogs = logCapture.filter(
        (log) => log.includes('DRY RUN') && log.includes('PumpPortal'),
      );

      expect(tradeLogs.length).toBeGreaterThan(0);

      // Verify details are logged
      const detailedLog = tradeLogs.find((log) => log.includes('buy') || log.includes('sell'));
  ...
  describe('Extended Simulation', () => {
    it('should handle multiple rapid transactions safely', async () => {
      ...
      const rapidLogs = logCapture.filter(
        (log) => log.includes('DRY RUN: PumpPortal') && tokens.some((token) => log.includes(token)),
      );
    ...
    it('should maintain consistent behavior over time', async () => {
      ...
      expect(
        sendPumpTrade.mock.results
          .slice(-5)
          .every((result: any) => result.value === null || (result.value && result.value.then)),
      ).toBe(true);
    });
  });
  ...
  describe('Logging Quality', () => {
    ...
    it('should not contain real transaction signatures', () => {
      ...
      const suspiciousLogs = logCapture.filter(
        (log) =>
  ...
  describe('Final Validation', () => {
    it('should complete comprehensive dry-run without real transactions', async () => {
      ...
      await sendPumpTrade({
        mint: 'FinalToken1111111111111111111111111111111',
        amount: 0.1,
        action: 'buy',
      });
      await sendPumpTrade({
      ...
      const finalLogs = logCapture.filter(
        (log) => log.includes('Final validation') || log.includes('FinalToken'),
      );
