...
import { runAutoSellLoop } from "../sell/autoSellManager.js";
...
class RiskManager {
    ...
    async checkPositionRisk({
        ...
        try {
            ...
            logger.error('RISK', 'Risk check failed', { mint, requestedAmount, error: (error as Error).message });
            return {
                allowed: false,
    ...
    private async updatePortfolioState(connection: Connection, walletPubkey: PublicKey): Promise<void> {
        try {
            ...
            // Get active positions count from auto-sell manager
            const autoSellStatus = runAutoSellLoop();
            this.portfolioState.activePositions = autoSellStatus.positions;
    ...
    shouldHaltTrading(): boolean {
        ...
        if (this.portfolioState.dailyLossTotal >= maxLossAmount) {
            logger.error('RISK', '🚨 EMERGENCY HALT: Daily loss percentage limit exceeded', {
                dailyLossPercent: (this.portfolioState.dailyLossTotal / this.portfolioState.walletBalance * 100).toFixed(1),
    ...
    getRiskSummary(): object {
        return {
            ...
            exposurePercent: ((this.portfolioState.totalExposure / this.portfolioState.walletBalance) * 100).toFixed(1),
            activePositions: this.portfolioState.activePositions,
