...
jest.mock("@solana/web3.js", () => {
    ...
    return {
        ...
        Connection: jest.fn().mockImplementation(() => ({
            ...
            sendTransaction: jest.fn().mockRejectedValue(new Error("DRY RUN: No real transactions allowed")),
            getAccountInfo: jest.fn().mockResolvedValue({
                data: Buffer.alloc(165), // Mock mint account data
                executable: false,
                lamports: 1000000,
                owner: new (jest.requireActual("@solana/web3.js").PublicKey)("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")
            })
        }))
    };
});
...
jest.mock("../../src/utils/pumpTrade.js", () => ({
    sendPumpTrade: jest.fn().mockImplementation(async (params) => {
        console.log(`🔥 DRY RUN: Would execute PumpPortal ${params.action} - Amount: ${params.amount}, Mint: ${params.mint}`);
...
describe("Dry-Run Validation Suite", () => {
    ...
    describe("No Real Transactions", () => {
        ...
        it("should log all would-be transactions with details", async () => {
            ...
            const transactionLogs = newLogs.filter(log => 
    ...
    describe("Extended Operation Simulation", () => {
        ...
        it("should handle error conditions gracefully", async () => {
            ...
            const errorLogs = logCapture.filter(log => 
                log.includes("[ERROR]") || log.includes("❌") || log.includes("Network timeout")
            );
    ...
    describe("Memory and Resource Management", () => {
        it("should not accumulate excessive log entries", () => {
            ...
            const duplicateLogs = logCapture.filter(log => 
                logCapture.filter(otherLog => otherLog === log).length > 10
            );
    ...
    describe("Final Validation", () => {
        it("should complete comprehensive dry-run cycle", async () => {
            ...
            const { sendPumpTrade } = require("../../src/utils/pumpTrade.js");
            const realTransactions = sendPumpTrade.mock.results.filter((result: any) => 
                result.value && !result.value.includes && result.value !== null
            );
            ...
            const finalDryRunCheck = logCapture.filter(log => 
                log.includes("DRY RUN") || log.includes("🔥")
            );
