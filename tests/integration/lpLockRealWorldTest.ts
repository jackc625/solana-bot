...
import { verifyLpLockStatus, verifyTokenLpLock, LpLockConfig } from "../../src/utils/lpLockVerification.js";
...
describe("LP Lock Verification - Real World Testing", () => {
    ...
    describe("Known LP Token Testing", () => {
        it("should handle completely burned LP tokens", async () => {
        ...
        it("should analyze well-known token LP pairs", async () => {
    ...
    describe("Error Handling and Edge Cases", () => {
        it("should handle non-existent mint addresses gracefully", async () => {
        ...
        it("should test performance with multiple concurrent requests", async () => {
            const testMints = [
                WELL_KNOWN_TOKENS.SOL,
                WELL_KNOWN_TOKENS.USDC,
                WELL_KNOWN_TOKENS.mSOL
            ];
            ...
            console.log(`Results: ${results.filter(r => r.status === 'fulfilled').length}/${results.length} successful`);
    ...
    describe("Configuration Sensitivity Testing", () => {
        it("should respond to different lock percentage requirements", async () => {
            ...
            for (const config of configs) {
                ...
                try {
                    ...
                    console.log(`Required: ${config.minLockPercentage}%, Result: ${result.lockPercentage}%, Passed: ${result.isLocked}`);
                } catch (error) {
        ...
        it("should handle accept/reject configuration options", async () => {
            const configs = [
            ...
            configs.forEach((config, index) => {
                console.log(`Config ${index + 1}: burnedLp=${config.acceptBurnedLp}, vestingLock=${config.acceptVestingLock}`);
            });
        }, NETWORK_TIMEOUT);
    });

    describe("Real Pool Discovery and Analysis", () => {
        it("should discover and analyze recent pools", async () => {
        ...
        it("should validate known safe vs risky pools", async () => {
