...
describe('validateEnvironment', () => {
  ...
  describe('PRIVATE_KEY validation', () => {
    ...
    test('should accept valid 64-byte base58 private key', () => {
      ...
      const validKey = 'So11111111111111111111111111111111111111112So11111111111111111111111111111111111111112'; // 88 chars base58 = ~64 bytes
  ...
  describe('RPC_URL validation', () => {
    beforeEach(() => {
      process.env.PRIVATE_KEY = 'So11111111111111111111111111111111111111112So11111111111111111111111111111111111111112';
    });
    ...
    test('should reject invalid URL format', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        expect.stringContaining('RPC_URL is not a valid URL')
      );
    });
  ...
  describe('optional environment variables', () => {
    ...
    test('should warn when NODE_ENV is not set', () => {
      ...
      expect(result.valid).toBe(true);
      expect(result.warnings).toContain(
        expect.stringContaining('NODE_ENV not set')
      );
    });
