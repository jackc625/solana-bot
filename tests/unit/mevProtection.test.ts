...
describe('MEV Protection System', () => {
  ...
  describe('MEV Risk Analysis', () => {
    test('should analyze low risk scenario correctly', async () => {
      ...
      (sandwichDetectionSystem.assessSandwichRisk as jest.MockedFunction<any>).mockResolvedValue(
        mockSandwichRisk,
      );
      (
        mevAwarePriorityFeeCalculator.calculateMEVAwareFee as jest.MockedFunction<any>
      ).mockResolvedValue(mockFeeCalculation);
      (
        mevAwarePriorityFeeCalculator.estimateNetworkMEVActivity as jest.MockedFunction<any>
  ...
  describe('Jito Bundle Integration', () => {
    ...
    test('should handle bundle submission success', async () => {
      ...
      (jitoBundleManager.submitBundle as jest.MockedFunction<any>).mockResolvedValue(
        mockBundleResult,
      );
  ...
  describe('Sandwich Attack Detection', () => {
    test('should detect suspicious mempool activity', async () => {
      const mockRiskAssessment = {
        ...
        recommendations: ['Use private mempool', 'Increase priority fees significantly'],
        shouldDelay: true,
        delayMs: 3000,
        shouldUsePrivateMempool: true,
      };
    ...
    test('should record MEV activity patterns', () => {
      ...
      expect(() => {
        sandwichDetectionSystem.recordMEVActivity(mockMint, pattern, botAddress, 0.05, 2.5);
      }).not.toThrow();
    });
  });
